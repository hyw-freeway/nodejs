...
![image-20220830100614313](C:\Users\14495\AppData\Roaming\Typora\typora-user-images\image-20220830100614313.png)

#### 创建数据库

![image-20220830100721500](C:\Users\14495\AppData\Roaming\Typora\typora-user-images\image-20220830100721500.png)

#### 创建数据表

![image-20220830101708479](C:\Users\14495\AppData\Roaming\Typora\typora-user-images\image-20220830101708479.png)

#### 插入数据

![image-20220830102305440](C:\Users\14495\AppData\Roaming\Typora\typora-user-images\image-20220830102305440.png)
...
![image-20220830105106759](C:\Users\14495\AppData\Roaming\Typora\typora-user-images\image-20220830105106759.png)
...
2. 插入数据
   ...
   ```
   const user = {username:'Bruce1', password:'555201'}
...
- 有利于 SEO（搜索引擎优化）。服务器响应的是完整的 HTML 页面内容，有利于爬虫爬取信息。
...
cookies：储存在用户浏览器中的一个不超过4kb的字符串。由名字，值和其他控制其有效期，安全性，适用范围的可选属性组成。

不同域名下的cookies是相互独立的。客户端发送请求时，会把当前域名下未过期的cookies发送到服务器。

特性：自动发送，域名独立，过期时限，4kb限制，不具有安全性。
...
- 服务器可以通过注册 express-jwt 全局中间件将客户端发送过来的 Token 解析还原成 JSON 对象
...
- 当 express-jwt 中间件配置成功后，即可在那些有权限的接口中，使用 req.user 对象，来访问从 JWT 字符串中解析出来的用户信息
  ...
  ```js
   app.get('/admin/getinfo', (req, res) => {
    ...
    console.log(req.user) // 里面的内容取决于加密为token时传入的用户信息对象
